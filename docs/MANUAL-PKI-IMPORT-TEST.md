...
When you import the CA cert, the router displays the CA fingerprint. Record it for verification or for use with `enrollment fingerprint` / `--pki-ca-fingerprint` (non-interactive auth). For the CA generated by `gen_pki_manual_test.py` (ca-cert.pem):
...
For the OpenSSL-generated root CA (**rootCA.pem**):
...
*Note: Regenerating PEMs with the script produces a new CA and new fingerprints.*
...
| Field | Meaning |
|-------|--------|
| **Certificate** (first block) | Router identity cert (CN=R1, R2, …). **Certificate Serial Number (hex)** is the cert’s serial. **Issuer** = CA (e.g. cn=TopoGen-Root-CA). **Subject** = this device (e.g. cn=R2). **Validity Date** = start/end. **Associated Trustpoints** = trustpoint name. **Storage** = nvram filename (e.g. `nvram:TopoGen-Root#711D.cer`). |
...
## 5. Alternative: OpenSSL-only generation
...
openssl x509 -req -in router.csr -CA rootCA.pem -CAkey rootCA.key -CAcreateserial \
  -out router.pem -days 3650 -sha256 -copy_extensions copyall
```
...
openssl req -x509 -new -nodes -key out/rootCA.key -sha256 -days 3650 -out out/rootCA.pem -subj "/CN=TopoGen-Root-CA"
```
...
openssl x509 -req -in out/r1.csr -CA out/rootCA.pem -CAkey out/rootCA.key -CAcreateserial -out out/r1.pem -days 3650 -sha256 -copy_extensions copyall
```
...
openssl x509 -req -in out/r2.csr -CA out/rootCA.pem -CAkey out/rootCA.key -CAcreateserial -out out/r2.pem -days 3650 -sha256 -copy_extensions copyall
