...
- Unreleased
  - fix(mgmt): emit `ipv6 unicast-routing` for static IPv6 OOB mode
    - Static IPv6 mode (`--mgmt-ipv6-static`) now emits `ipv6 unicast-routing` in the day-0 config, consistent with SLAAC/DHCPv6, reversing the prior "IPv6 host only" behavior. Root cause was a `!= "static"` guard in `_render_bootstrap_config()` (NaC `--bootstrap` path); the line is placed immediately after the DNS / `ip domain name` block. Full-template paths (`csr-ospf`, `csr-dmvpn`, `iosv-eigrp`) gain the same conditional emit and the redundant block was removed from the OOB partials (`_csr_mgmt_oob`, `_iosv_mgmt_oob`) to avoid double emission. Validated across the full 15-lab matrix (simple/nx/flat/flat-pair/dmvpn × static/slaac/dhcpv6) — all emit `ipv6 unicast-routing`; all 15 imported to CML 2.10 (192.168.1.183).
    - Files: src/topogen/render.py (rev v1.5.2), src/topogen/templates/_csr_mgmt_oob.jinja2 (rev v1.0.2), src/topogen/templates/_iosv_mgmt_oob.jinja2 (rev v1.0.2), src/topogen/templates/csr-ospf.jinja2 (rev v1.3.3), src/topogen/templates/csr-dmvpn.jinja2 (rev v1.4.3), src/topogen/templates/iosv-eigrp.jinja2 (rev v1.1.4), scripts/import-ipv6-matrix-labs.py (new), README.md (rev v1.8.13), CHANGES.md (rev v1.3.21)
  - feat(mgmt): static IPv6 OOB from explicit `/64` anchor with FF10 embedding (TG-195)
    - `--mgmt-ipv6-static` + required `--mgmt-ipv6-cidr /64` render deterministic global `ipv6 address` on IOSv/CSR OOB (routers only). Optional `--mgmt-ipv6-static-link-local` adds loopback-derived `fe80::FF10:…` on OOB. Static mode emits `ipv6 unicast-routing` (see Unreleased fix above; original TG-195 deferred it). NaC `ansible_host` populated at generate time; `sync-nac-mgmt.py` skips live poll when `mgmt_ipv6_mode: static`. Examples use `fd80::/64` or `2001:db8:1:2::/64` only.
