...
- [ ] **Task: Bump `pyproject.toml` version to `0.2.5`.** Branch `release/0.2.5` still has `version = "0.2.4"` in `pyproject.toml`. Generated lab descriptions show `Generated by topogen v0.2.4` instead of `v0.2.5`. Fix: update `pyproject.toml` `version` field to `"0.2.5"` and reinstall (`pip install -e .`). Separate branch — do not bundle with feature/fix branches.
